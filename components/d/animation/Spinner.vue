...
<template>
  <svg class="spinner" viewBox="0 0 630 630" xmlns="http://www.w3.org/2000/svg" :style="{
    '--d-spinner-spin-period': spinPeriod + 's',
    '--d-spinner-blade-wiggle-period': bladeWigglePeriod + 's'
  }">
    <defs>
      <DAnimationDefLetterD id="d-letter" />
      <use href="#d-letter" id="d-blade" class="d-animation-spinner__blade" :transform-origin="transformOrigin" />
    ...
    <use href="#spinner" class="fill-white" :transform-origin="transformOrigin" />
  </svg>
</template>
...
.d-animation-spinner__blade {
  ...
  animation: d-spinner-blade-wiggle var(--d-spinner-blade-wiggle-period) ease-in-out infinite;
}
